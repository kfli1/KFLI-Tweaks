...
<#
  PULSE | Gaming Tweak - GUI Edition v1.2 (WPF)
  ...
  What changed vs v1.1:
   ...
   - Added support for `iwr <url> | iex` by materializing a temporary copy before elevation.
...
# ---------------------------------------------------------------------------
# IEX / `iwr <url> | iex` bootstrap
# ---------------------------------------------------------------------------
# When PowerShell receives this script through Invoke-Expression, there is no
# reliable on-disk $PSCommandPath.  Instead of trying to reconstruct the
# ScriptBlock (which is unreliable under IEX), download this script from its
# canonical GitHub URL, save it to %TEMP%, and relaunch that real file.
# IMPORTANT: update this URL if the repository/file is moved or renamed.
$script:LauncherUrl = 'https://raw.githubusercontent.com/kfli1/KFLI-Tweaks/main/KFLI%20Gaming%20Tweak.ps1'
$script:BootstrapFromIex = $false

if ([string]::IsNullOrWhiteSpace($ScriptPath)) {
    try {
        $bootstrapRoot = Join-Path $env:TEMP 'KFLI-Tweaks'
        New-Item -ItemType Directory -Path $bootstrapRoot -Force -ErrorAction Stop | Out-Null
        $candidate = Join-Path $bootstrapRoot 'KFLI Gaming Tweak.ps1'

        # Download the canonical script to disk. This makes relative paths,
        # logs, backups, WPF resources, and self-elevation work normally.
        Invoke-WebRequest -UseBasicParsing -Uri $script:LauncherUrl `
            -OutFile $candidate -ErrorAction Stop

        if (-not (Test-Path -LiteralPath $candidate)) {
            throw 'The downloaded launcher file was not created.'
        }

        $ScriptPath = $candidate
        $script:BootstrapFromIex = $true
    }
    catch {
        Show-Fatal ('Could not start KFLI Gaming Tweak from the launcher URL.' +
            "`n`n" + $_.Exception.Message +
            "`n`nDownload the .ps1 file and run it directly if the problem continues.")
        exit 1
    }
}

if ([string]::IsNullOrWhiteSpace($ScriptPath) -or -not (Test-Path -LiteralPath $ScriptPath)) {
    Show-Fatal 'Could not determine the script path. Download the file and run it directly, or use the official launcher URL.'
    exit 1
}
...
function Disable-Nagle {
    Invoke-Step "Nagle's algorithm" {
        ...
        foreach ($k in @(Get-ChildItem -LiteralPath $root)) {
            Set-Reg $k.PSPath 'TcpAckFrequency' 1
            Set-Reg $k.PSPath 'TCPNoDelay' 1
            $n++
        }
        Write-Log ("Nagle's algorithm disabled on {0} network interface(s)" -f $n) 'OK'
    }
}
    ...
    Invoke-Step 'Temp cleanup' {
        ...
        foreach ($p in @($env:TEMP, (Join-Path $env:WINDIR 'Temp'))) {
            ...
            foreach ($item in @(Get-ChildItem -LiteralPath $p -Force -ErrorAction SilentlyContinue)) {
                $skip = $false
                foreach ($k in $keep) {
                    if ($k.StartsWith($item.FullName, [System.StringComparison]::OrdinalIgnoreCase)) { $skip = $true }
                }
                if ($skip) { continue }
...
$Actions = @{
    ...
    cpu   = @{ Title = 'CPU and RAM Tweak'
               Cmd = 'Set-PowerPlanUltimate; Set-GamesTaskPriority; Write-Done "CPU and RAM Tweak"' }
    ...
    qtemp = @{ Title = 'Clear Temp Files';  Cmd = 'Invoke-TempCleanup; Write-Done "Clear Temp Files"' }
    qdns  = @{ Title = 'Flush DNS';         Cmd = 'Invoke-NetworkCleanup; Write-Done "Flush DNS"' }
    qnet  = @{ Title = 'Optimize Network';  Cmd = 'Disable-Nagle; Write-Done "Optimize Network"' }
...
$Tiles = @(
    ...
    @{ Id = 'cpu';     N = 6; Name = 'CPU & RAM TWEAK';      Badge = 'BOOST';           Desc = 'Performance Boost';               Tip = 'Ultimate Performance plan and boosted game scheduling priority.' },
    @{ Id = 'net';     N = 7; Name = 'NETWORK TWEAK';        Badge = 'STABILITY';       Desc = 'Lower Ping - Better Stability';   Tip = 'Flushes DNS and disables Nagle algorithm on all network interfaces.' },
