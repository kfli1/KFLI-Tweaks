...
# Support `iwr <url> | iex` as well as normal .ps1 execution.
# Invoke-Expression does not give us a real $PSCommandPath, but the current
# ScriptBlock can be written to a temporary .ps1 so the rest of the application
# (self-elevation, relative files, logs and backups) can work exactly as it does
# when the script is launched from disk.
$script:BootstrapFromIex = $false
if ([string]::IsNullOrWhiteSpace($ScriptPath)) {
    try {
        $iexText = $null
        try { $iexText = $MyInvocation.MyCommand.ScriptBlock.ToString() } catch {}
        if ([string]::IsNullOrWhiteSpace($iexText)) {
            try { $iexText = [string]$MyInvocation.MyCommand.Definition } catch {}
        }
        if (-not [string]::IsNullOrWhiteSpace($iexText) -and $iexText.Length -gt 256) {
            ...
            Set-Content -LiteralPath $candidate -Value $iexText -Encoding UTF8 -Force -ErrorAction Stop
